                                    ...
                                    <table id="example1" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
                                        ...
                                        <tbody>
                                            <tr v-for="item in items.data" :key="item.id">
                                                ...
                                                <td> {{ mahanagars[item.mahanagar_id] }} </td>
                                                <td v-if="item.image"> <img class="img-bordered-ssm" :src="getImage(item.image)" width="100%" height="25" alt="Thana image" /> </td>
                                                <td v-else> <img class="img-bordered-ssm" src="/assets/img/img404.png" width="100%" height="25" alt="Thana image" /> </td>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 v-show="editMode" class="modal-title" id="addNewModelLabel">Update Thana</h5>
                        <h5 v-show="!editMode" class="modal-title" id="addNewModelLabel">Add Thana</h5>
                    ...
                    <form @submit.prevent="editMode ? update() : create()" @keydown="form.onKeydown($event)" enctype="multipart/form-data">
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="mahanagar_id">Mahanagar<span class="required">*</span>:</label>
                                <select v-model="form.mahanagar_id" name="mahanagar_id" id="mahanagar_id"
                                class="form-control" :class="{ 'is-invalid': form.errors.has('mahanagar_id') }" >
                                <option value="">Select Mahanagar</option>
                                    <option v-for="(mahanagar, index) in mahanagars" :value="index" :key="index"> {{ mahanagar }} </option>
                                </select>
                                <has-error :form="form" field="mahanagar_id"></has-error>
                            </div>
...
<script>
    export default {
        ...
        methods: {            
            ...
            loadData(){
                axios.get('api/thana').then(({ data }) => (
                    this.mahanagars = data [0],
                    this.items = data [1]
                ));
            },
