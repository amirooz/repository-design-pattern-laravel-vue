                                    ...
                                    <table id="example1" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
                                        ...
                                        <tbody>
                                            <tr v-for="item in items.data" :key="item.id">
                                                ...
                                                <td v-if="item.image"> <img class="img-bordered-ssm" :src="getImage(item.image)" width="100%" height="25" alt="Pourashava image" /> </td>
                                                <td v-else> <img class="img-bordered-ssm" src="/assets/img/img404.png" width="100%" height="25" alt="Pourashava image" /> </td>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 v-show="editMode" class="modal-title" id="addNewModelLabel">Update Pourashava</h5>
                        <h5 v-show="!editMode" class="modal-title" id="addNewModelLabel">Add Pourashava</h5>
                    ...
                    <form @submit.prevent="editMode ? update() : create()" @keydown="form.onKeydown($event)" enctype="multipart/form-data">
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                placeholder="Enter pourashava name..."
                            ...
                            <div class="form-group">
                                ...
                                class="form-control" :class="{ 'is-invalid': form.errors.has('district_id') }" >
                                    ...
                                    <option v-for="(district, index) in districtList" :value="index" :key="index"> {{ district }} </option>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select v-model="form.upazilla_id" name="upazilla_id" id="upazilla_id"
                                class="form-control" :class="{ 'is-invalid': form.errors.has('upazilla_id') }" >
                                <option value="">Select Upazill</option>
                                    <option v-for="(upazilla, index) in upazillaList" :value="index" :key="index"> {{ upazilla }} </option>
                                </select>
...
<script>
    export default {
        ...
        methods: {            
            ...
            loadData(){
                axios.get('api/pourashava').then(({ data }) => (
            ...
            pagination(page = 1) {
                axios.get('api/pourashava?page=' + page)
                .then(response => {
            ...
            getImage(img){
                return "assets/img/pourashava/" + img;
            },
            ...
            update() {
                this.$Progress.start();
                this.form.put('api/pourashava/' + this.form.id)
                .then(() => {
                    $('#addNewModel').modal('hide');
                    Fire.$emit('reloadData');
                    Toast.fire({
            ...
            deleteData(id) {
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete('api/pourashava/' + id)
                        .then(() => {
                            Fire.$emit('reloadData');
